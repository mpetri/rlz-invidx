...
public:
    ...
    using factorization_strategy = factorizor<t_factorization_block_size, t_search_local_block_context, factor_encoder>;
...
public:
    ...
    rlz_store build_or_load(collection& col,std::string input_file,std::string name) const
    {
        ...
        if (rebuild || !utils::file_exists(dict_file_name)) {
            dict = dictionary_creation_strategy::create(input_file, dict_size_bytes);
        } else {
        ...
        if (rebuild || !utils::file_exists(factor_file_name)) {
            factorization_strategy::parallel_factorize(col,input_file,dict,hash,num_threads);
        }
        else {
        ...
        return rlz_store(col,input_file,dict_hash,input_hash);
    }
private:
    bool rebuild = false;
